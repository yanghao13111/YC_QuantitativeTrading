...
def screen_stock(symbol, data_folder):
    ...
    condition = (
        ...
        (abs((df['EMA5'] - df['EMA10']) / df['EMA10']) > 0.01) & 
        (abs((df['EMA5'] - df['EMA10']) / df['EMA10']) < 0.05) &	
        (abs((df['EMA10'] - df['EMA22']) / df['EMA22']) > 0.01) &	
        (abs((df['EMA10'] - df['EMA22']) / df['EMA22']) < 0.05) &
        (abs((df['EMA22'] - df['EMA66']) / df['EMA66']) > 0.01) &	
        (abs((df['EMA22'] - df['EMA66']) / df['EMA66']) < 0.05) &
        (abs((df['EMA66'] - df['EMA264']) / df['EMA264']) < 0.1) &	
        (df['Trading_Volume'] > 500*1000) &	
