...
def get_dates():
    start_date = datetime(2015, 1, 1)  # 指定年份为2022，月份为1，日期为1
...
def main():
    config = {
        'symbol': '2376.TW',
        'timeframe': '1d',  
        'buy_expression': 'self.ema5[0] > self.ema5[-1] and self.ema10[0] > self.ema10[-1] and self.ema22[0] > self.ema22[-1] and self.ema66[0] > self.ema66[-1] and self.volume[0] > 2 * self.volume[-1] and self.ema5[0] > self.ema10[0] and self.ema10[0] > self.ema22[0] and self.ema22[0] > self.ema66[0] and abs((self.ema5[0] - self.ema10[0]) / self.ema10[0]) < 0.03 and abs((self.ema10[0] - self.ema22[0]) / self.ema22[0]) < 0.03',
        'sell_expression': 'self.ema22[0] < self.ema22[-1]'
    }
